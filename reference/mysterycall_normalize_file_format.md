...
``` r
mysterycall_normalize_file_format(format = NULL, path = NULL, default = "csv")
```
...
- format:
  ...
  Optional character scalar: "csv" or "parquet".

- path:

  Optional character scalar. File path to infer format from.

- default:

  Character scalar to return if format cannot be inferred.

## Value

Character scalar: "csv" or "parquet".
