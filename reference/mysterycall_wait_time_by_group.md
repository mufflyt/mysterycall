...
- outcome_col:
  ...
  Character scalar. Column containing wait-time values (numeric).
  Default `"business_days_until_appointment"`.

- group_col:

  Character scalar. Column to group by (e.g. `"scenario"`).

- digits_median:

  Integer. Rounding digits for the median. Default `1`.

- digits_q:

  Integer. Rounding digits for Q1 and Q3. Default `0`.

- filter_positive:

  Logical. If `TRUE`, pre-filter rows where `outcome_col > 0` before
  computing statistics. Default `FALSE`.
...
- `<group_col>`:
  ...
  The grouping variable values.

- `median_days`:

  Median wait time (rounded to `digits_median`).

- `q1`:

  25th percentile (rounded to `digits_q`).

- `q3`:

  75th percentile (rounded to `digits_q`).

- `n`:

  Non-NA count per group.

## See also

[`mysterycall_wait_time_summary()`](https://mufflyt.github.io/mysterycall/reference/mysterycall_wait_time_summary.md),
which reports the same median/Q1/Q3 columns plus mean/SD/range and a
group-comparison test with an interpretation sentence. Use this leaner
helper when you only need the quantile table (with optional CSV export).
