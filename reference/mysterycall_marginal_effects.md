...
- model:
  ...
  lme4, a `mysterycall_poisson_model`, or a `mysterycall_nb_model`
  ...
  effects are computed via
  [`marginaleffects::avg_slopes()`](https://rdrr.io/pkg/marginaleffects/man/slopes.html)
  and the result is coerced into the standard `term` / `level` / `ame` /
  `variable_type` data frame returned by this function.
...
[`marginaleffects::avg_slopes()`](https://rdrr.io/pkg/marginaleffects/man/slopes.html)
for direct use with glmmTMB NB models.
