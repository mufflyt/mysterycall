...
When DHARMa is installed (recommended), uses randomized quantile
residuals via
[`DHARMa::simulateResiduals()`](https://rdrr.io/pkg/DHARMa/man/simulateResiduals.html) -
the current gold standard for diagnosing GLMMs. Falls back to Pearson
residual ggplot2 panels when DHARMa is unavailable or
`use_dharma = FALSE`.
...
- model:
  ...
  A `mysterycall_poisson_model` or `mysterycall_nb_model` result, or a
  bare `glm`, `glmerMod`, `glmmTMB`, or `lm` object.

- use_dharma:

  Logical. When `TRUE` (default) and DHARMa is installed, uses
  [`DHARMa::simulateResiduals()`](https://rdrr.io/pkg/DHARMa/man/simulateResiduals.html)
  for residual diagnostics. Set to `FALSE` to force the Pearson residual
  ggplot2 panels.

- n_sim:

  Integer. Number of simulations passed to
  [`DHARMa::simulateResiduals()`](https://rdrr.io/pkg/DHARMa/man/simulateResiduals.html).
  Higher values give smoother distributions at the cost of time. Default
  `250L`.

- plot:

  Logical. When `TRUE` (default), draws the DHARMa diagnostic plot to
  the active graphics device. Ignored on the Pearson path (those plots
  are always returned, never auto-drawn).
...
**DHARMa path** (`use_dharma = TRUE` and DHARMa installed): Returns the
`DHARMaResiduals` simulation object invisibly. Pass it to
...
**Pearson path** (`use_dharma = FALSE` or DHARMa not installed): Returns
...
for a targeted zero-inflation test;
...
to write Pearson-path plots to disk.
