...
Where the GLMM estimates a *conditional* (within-practice) log-odds and
needs each cluster's random intercept to be estimable, a generalized
estimating equation (GEE) estimates the *population-average* (marginal)
log-odds and treats the within-practice correlation as a nuisance to be
modelled – so it happily uses every record, including the singleton and
dyad practices a matched analysis would drop. The
marginal-versus-conditional contrast is a standard sensitivity analysis
for clustered binary access outcomes: agreement in sign and rough
magnitude across the two targets of inference is reassuring, divergence
is worth a sentence in the paper. Robust ("sandwich") standard errors
make the result insensitive to the working-correlation choice.
...
- outcome:
  ...
  Character scalar naming the binary outcome column. Coerced with
  [`as.integer()`](https://rdrr.io/r/base/integer.html) after mapping
  `positive_values` to 1; anything else to 0.

- predictors:

  Character vector of predictor column names. The model is
  `outcome ~ predictors[1] + ... `.

- cluster:

  Character scalar naming the cluster / practice id column (the GEE
  `id`). Rows are sorted by this internally, as `geeglm()` requires.

- positive_values:

  Value(s) of `outcome` counted as the event. Default `TRUE` (also
  matches `1`, `"TRUE"`, `"Yes"` via `%in%`).

- corstr:

  Working correlation structure passed to
  [`geepack::geeglm()`](https://rdrr.io/pkg/geepack/man/geeglm.html).
  Default `"exchangeable"`.

- reference:

  Optional named character vector mapping a predictor name to the factor
  level to use as its reference (via
  [`stats::relevel()`](https://rdrr.io/r/stats/relevel.html)), e.g.
  `c(scenario = "Straight couple")`. Default `NULL`.

- conf_level:

  Confidence level for the odds-ratio CI. Default `0.95`.

- output_dir, filename:

  Optional CSV export of the coefficient table. `output_dir = NA`
  (default) skips writing; `NULL` writes to a session temp directory via
  [`mysterycall_tempdir()`](https://mufflyt.github.io/mysterycall/reference/mysterycall_tempdir.md).

## Value

An object of class `"mysterycall_gee"`: a list with `model` (the fitted
`geeglm`), `or_table` (a
[`tibble::tibble()`](https://tibble.tidyverse.org/reference/tibble.html)
of `term`, `estimate` log-OR, robust `se`, `wald_z`, `p_value`, `or`,
`ci_lower`, `ci_upper`), `corstr`, `n`, `n_clusters`, and `separated`
(terms whose \|log-OR\| exceeds 10, a complete-separation flag
suggesting a penalized fit instead).
[`as.data.frame()`](https://rdrr.io/r/base/as.data.frame.html) returns
the `or_table`.

## Details

Requires the geepack package.
...
#> Population-average GEE (binomial/logit, exchangeable)  n = 60  clusters = 30
#>                    term  OR              CI       p
#>             (Intercept) 5.0 [1.914, 13.061] 0.00102
#>  scenarioLesbian couple 0.3  [0.100, 0.897] 0.03120
# }
```
