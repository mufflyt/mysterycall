...
- outcome_col:
  ...
  Character scalar. Binary 0/1 outcome column. Default `"offered"`.

- date_col:

  Character scalar or `NULL`. Date column used to detect calendar drift.
  Default `NULL`.

- call_seq_col:

  Character scalar or `NULL`. Integer column giving the sequential call
  number for each caller (1st call, 2nd call, etc.). Default `NULL`.
  When `NULL` and both `date_col` and `caller_col` are provided, the
  sequence is derived by ranking dates within each caller (ties broken
  by row order).

- caller_col:

  Character scalar or `NULL`. Grouping column for caller identity, used
  to derive `call_seq_col` when that argument is `NULL`. Default `NULL`.

- week_or_day:

  Character scalar. Aggregation unit for the calendar trend: `"week"`
  (default) or `"day"`.

- plot:

  Logical. When `TRUE` (default) and ggplot2 is installed, `$plot` in
  the return value is a `ggplot` object; otherwise `$plot` is `NULL`.

- test:

  Character scalar. Trend-test method. Currently only `"lm"` (default)
  is supported.

## Value

A named list of class `mysterycall_caller_drift`:

- `calendar`:

  Named list or `NULL` (when `date_col` is `NULL`). Elements:
  `rate_by_period` (data frame with columns `period`, `time_index`,
  `rate`, `n`), `slope`, `slope_se`, `p_value`, `p_fmt`, `sentence`.

- `sequence`:

  Named list or `NULL` (when no sequence information is available).
  Elements: `rate_by_seq` (data frame with columns `seq_bin`,
  `mean_seq`, `rate`, `n`), `slope`, `slope_se`, `p_value`, `p_fmt`,
  `sentence`.
...
- `sentence`:
  ...
  Character scalar. Combined one-sentence summary ready to paste into
  the methods section, e.g. *"Acceptance rates did not change
  significantly over the study period (slope=-0.002 per week, p=0.43) or
  over call sequence (slope=0.001 per call, p=0.81), supporting design
  validity."*

## Details

**Calendar drift.** `outcome_col` is aggregated by `week_or_day` period.
A linear model `lm(rate \~ time_index)` is fitted, where `time_index` is
the chronological rank of each period (1, 2, 3, ...). The slope is the
estimated change in acceptance rate per week (or day). Week boundaries
follow `format(date, "%Y-%U")` (Sunday-start ISO-adjacent convention).

**Sequence drift.** Calls are grouped into consecutive bins of five by
`call_seq_col` (or by the within-caller date rank when `call_seq_col` is
`NULL`). Within each bin the acceptance rate and mean sequence number
are computed. A linear model `lm(rate \~ mean_seq)` is fitted; the slope
is the estimated change in acceptance rate per additional call.
...
#>    period time_index      rate n
#> 1 2023-01          1 0.3333333 6
#> 2 2023-02          2 0.2857143 7
#> 3 2023-03          3 0.5714286 7
#> 4 2023-04          4 0.5714286 7
#> 5 2023-05          5 0.2857143 7
#> 6 2023-06          6 0.5714286 7
