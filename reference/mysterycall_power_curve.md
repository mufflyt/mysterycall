...
for every combination of `n_range` and `irr_values`, collects the
results, and returns a publication-ready ggplot2 figure showing how
power grows with sample size under each IRR scenario.

## Usage

``` r
mysterycall_power_curve(
  n_range = seq(20, 100, by = 10),
  irr_values = c(0.6, 0.7, 0.75, 0.8),
...
- n_range:
  ...
  Integer vector. Physician counts per group to evaluate. Default
  `seq(20, 100, by = 10)`.

- irr_values:

  Numeric vector. IRR scenarios to plot (e.g. `c(0.60, 0.75, 0.80)`).
  Must all be positive. Default `c(0.60, 0.70, 0.75, 0.80)`.

- calls_per_physician:

  Integer. Calls per physician. Default `2L`.

- theta:

  Numeric. NB dispersion parameter. Default `2.0`.

- baseline_mean:

  Numeric. Reference-group mean wait days. Default `21`.

- sigma_u:

  Numeric. SD of physician random intercept (log scale). Default `0.3`.
...
- n_sim:
  ...
  Integer. Monte Carlo simulations per cell. Default `500L`. Use
  `>= 1000` for publication.

- seed:

  Integer. Random seed passed to each
  [`mysterycall_nb_power()`](https://mufflyt.github.io/mysterycall/reference/mysterycall_nb_power.md)
  call. Default `42L`.

- target_power:

  Numeric in (0, 1). Dashed reference line position and threshold used
  to compute `$min_n`. Default `0.80`.
...
A list of class `mysterycall_power_curve` (returned invisibly):
...
- `data`:
  ...
  Data frame with columns `n_physicians`, `irr_label`, `irr`, `power`,
  `ci_lower`, `ci_upper`.

- `min_n`:

  Named numeric. Minimum N achieving `target_power` for each IRR
  scenario; `NA` when never achieved across `n_range`.
...
#> Simulating N = 20, IRR = 0.70 ...
#> Simulating N = 40, IRR = 0.70 ...
#> Simulating N = 60, IRR = 0.70 ...
#> Simulating N = 20, IRR = 0.80 ...
#> Simulating N = 40, IRR = 0.80 ...
#> Simulating N = 60, IRR = 0.80 ...
```
