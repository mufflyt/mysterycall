...
Returns a broom-compatible tibble of fixed-effect incidence rate ratios
(IRR) with Wald 95% CIs and p-values. Column names follow broom
conventions so the result integrates with tidy workflows.
...
- x:
  ...
  A `mysterycall_poisson_model` object returned by
...
with one row per fixed-effect term and columns:
...
- `estimate`:
  ...
  IRR (incidence rate ratio) on the exponentiated scale.

- `std.error`:

  Standard error of the *log*-IRR.
...
- `p.value`:
  ...
  Two-tailed Wald p-value.

- `conf.low`:

  Lower bound of 95% Wald CI for IRR.

- `conf.high`:

  Upper bound of 95% Wald CI for IRR.
...
#> 1 scenarioB      1.5      0.15       2.6   0.009      1.1         2
# }
```
