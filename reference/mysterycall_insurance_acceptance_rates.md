...
appointment), distinct-phone denominator (total minus excluded), and a
...
- insurance_col:
  ...
  Character scalar. Default `"insurance"`.

- exclusion_col:

  Character scalar. Default `"reason_for_exclusions"`.

- days_col:

  Character scalar. Default `"business_days_until_appointment"`.

- medicaid_accept_col:

  Character scalar. Column used to screen for NA before counting
  acceptances. Default `"does_the_physician_accept_medicaid"`.

- phone_col:

  Character scalar. Distinct unit for counting. Default `"phone"`.
...
- medicaid_label:
  ...
  Character scalar. Default `"Medicaid"`.

- bcbs_label:

  Character scalar. Default `"Blue Cross/Blue Shield"`.

- digits:

  Integer. Decimal places for rates. Default `1`.
...
- `count_accepts_medicaid`:
  ...
  Numerator: Medicaid physicians who accepted and gave an appointment.

- `count_medicaid_excluded`:

  Distinct excluded Medicaid phones.

- `total_count_medicaid`:

  Total distinct Medicaid phones.

- `count_medicaid_denom`:

  Effective denominator (total minus excluded).
...
- `count_bcbs_excluded`:
  ...
  Distinct excluded BCBS phones.

- `total_count_bcbs`:

  Total distinct BCBS phones.

- `count_bcbs_denom`:

  Effective denominator: BCBS.

- `bcbs_acceptance_rate`:

  Rounded percentage.

- `paragraph`:

  Full manuscript paragraph with both rates.

- `summary_sentence`:

  Shorter inline sentence for embedding.
...
for overall sample counts;
[`mysterycall_acceptance_rate_calc()`](https://mufflyt.github.io/mysterycall/reference/mysterycall_acceptance_rate_calc.md)
for a simpler rate helper.
