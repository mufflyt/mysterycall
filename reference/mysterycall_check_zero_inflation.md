...
to assess whether the observed count of zeros in the outcome is higher
than expected under the fitted distribution. Applicable to both
...
``` r
mysterycall_check_zero_inflation(model, n_sim = 500L, plot = TRUE)
```
...
- model:
  ...
  A `mysterycall_poisson_model`, `mysterycall_nb_model`, or bare
  `glmerMod` / `glmmTMB` object.

- n_sim:

  Integer. Number of simulations for DHARMa. Default `500L`.

- plot:

  Logical. When `TRUE` (default), print the DHARMa zero-inflation plot
  to the active graphics device.

## Value

Invisibly returns the DHARMa `testZeroInflation` result list (class
`"DHARMaTest"`), which contains `$statistic`, `$p.value`, and the
simulated distribution. Prints a one-line summary to the console.

## Details

A significant result (p \< 0.05) means the model predicts fewer zeros
than observed — consider a zero-inflated Poisson or zero-inflated
negative binomial model (e.g.
...
- `p > 0.05`:
  ...
  No evidence of zero-inflation under the fitted model.

- `p < 0.05`:

  Significant zero-inflation detected. The model may underestimate the
  probability of zero-wait appointments.

## See also

[`mysterycall_plot_residuals()`](https://mufflyt.github.io/mysterycall/reference/mysterycall_plot_residuals.md)
for full DHARMa residual diagnostics;
[`mysterycall_nb_model()`](https://mufflyt.github.io/mysterycall/reference/mysterycall_nb_model.md)
for the negative binomial model that handles overdispersion (but not
zero-inflation).
...
#> Model fitted: n=60, physicians=10, AIC=347.3, overdispersion=1.11
mysterycall_check_zero_inflation(fit, plot = FALSE)
#> No zero-inflation detected (p = 1.000).
```
