...
``` r
# S3 method for class 'mysterycall_logistic_model'
tidy(x, ...)
```
...
- `estimate`:
  ...
  OR (odds ratio) on the exponentiated scale.
...
- `conf.low`:
  ...
  Lower bound of 95% Wald CI for OR.

- `conf.high`:

  Upper bound of 95% Wald CI for OR.
...
#> 1 scenarioB      2.5       0.3       3.1   0.002      1.8       3.5
# }
```
