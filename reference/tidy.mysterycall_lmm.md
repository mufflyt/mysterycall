...
- `estimate`:
  ...
  Point estimate (days, or log-days if log-transformed).
...
- `p.value`:
  ...
  Two-tailed Satterthwaite p-value.

- `conf.low`:

  Lower bound of Wald CI.

- `conf.high`:

  Upper bound of Wald CI.
...
#> 1 scenarioB      2.1       0.5       4.2    80   0.001      1.1       3.1
# }
```
