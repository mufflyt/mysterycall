...
Extends the primary model's print output with the routing decision, LMM
defensibility verdict, and the overall recommendation.

## Usage

``` r
# S3 method for class 'mysterycall_auto_model'
print(x, digits = 3, ...)
```
...
- x:
  ...
  A `mysterycall_poisson_model` or `mysterycall_nb_model` returned by
  [`mysterycall_auto_model()`](https://mufflyt.github.io/mysterycall/reference/mysterycall_auto_model.md),
  carrying a `$selection` element.

- digits:

  Integer. Decimal places for coefficients. Default `3`.
