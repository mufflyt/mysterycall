...
Computes overall and per-group median/IQR wait times, fits a Poisson GLM
to obtain p-values for each non-reference level versus the reference,
and assembles a manuscript-ready paragraph combining all pieces.
...
- outcome_col:
  ...
  Character scalar. Numeric wait-time column. Default
  `"business_days_until_appointment"`.

- group_col:

  Character scalar. Grouping variable (e.g. `"scenario"`).

- reference:

  Character scalar or `NULL`. Reference level for the Poisson model.
  `NULL` defaults to the first level alphabetically.

- filter_positive:

  Logical. If `TRUE`, pre-filter rows where `outcome_col > 0`. Default
  `FALSE`.

- digits_median:

  Integer. Rounding digits for medians. Default `0`.

- digits_p:

  Integer. Significant digits for p-values. Default `3`.
...
- `sentence`:
  ...
  Character scalar – the full manuscript paragraph.

- `overall_stats`:

  Named list: `median`, `q1`, `q3` (across all rows).
...
- `p_values`:
  ...
  Named numeric vector of formatted p-values (one per non-reference
  level).
...
cat(res$sentence)
#> The median wait time across all scenario was 16 business days (IQR: 10–17). Specifically, the median wait time was 8 days (IQR: 7–10) for BCBS, 16 days (IQR: 15–17) for Medicaid, 20 days (IQR: 16–21) for Self-Pay. The p-value for Medicaid vs BCBS was < 0.001. The p-value for Self-Pay vs BCBS was < 0.001.
```
