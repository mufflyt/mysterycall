...
```{r load-package, eval=FALSE, purl=FALSE}
library(mysterycall)
```
...
appointment. Because the callers follow a standardized script, vary only the
...
Medicaid patients wait longer for an orthopedic consultation? Are uninsured
...
2. **Wait time in business days (count ≥ 0).**  Among calls that resulted in an
   ...
   This is a non-negative integer count and is analyzed with a Poisson
...
physician share the physician's baseline acceptance behavior, scheduling culture,
...
generalized linear mixed-effects model (GLMM) approach, implemented in R by the
`lme4` package.
...
```{r preflight-example, eval=FALSE, purl=FALSE}
...
```{r preflight-fix, eval=FALSE, purl=FALSE}
...
```{r wait-time-dist, echo=TRUE, eval=TRUE, fig.width=6.5, fig.height=3.2, fig.cap="Right-skewed wait-time distribution for Medicaid callers (simulated). `mysterycall_plot_distribution()` returns raw and sqrt-scaled panels — the sqrt panel makes the long upper tail visible without distorting the modal value.", purl=FALSE}
set.seed(42)
...
scheduling behavior that is *not* explained by the covariates. Formally, the
model is:
...
```{r poisson-basic, eval=FALSE, purl=FALSE}
...
```{r irr-table, eval=FALSE, purl=FALSE}
# Print the IRR table
...
```{r poisson-print, eval=FALSE, purl=FALSE}
print(poisson_fit)
...
```{r irr-forest, echo=TRUE, eval=TRUE, fig.width=5.5, fig.height=3.0, fig.cap="Forest plot of incidence rate ratios (IRR) from the Poisson GLMM produced by `mysterycall_irr_plot()`. Red points are statistically significant (p < 0.05); navy are not. Dashed line at IRR = 1 marks no effect.", purl=FALSE}
...
```{r overdispersion, eval=FALSE, purl=FALSE}
...
```{r convergence, eval=FALSE, purl=FALSE}
...
```{r offset-example, eval=FALSE, purl=FALSE}
...
```{r wait-summary, eval=FALSE, purl=FALSE}
...
```{r combine-tables, eval=FALSE, purl=FALSE}
...
```{r acceptance-dot, echo=TRUE, eval=TRUE, fig.width=5.5, fig.height=3.2, fig.cap="Appointment-offered rates by insurance type with 95 % Wilson score confidence intervals. Private insurance is the reference group (dashed line). Medicaid and Uninsured callers face the largest absolute deficits.", purl=FALSE}
...
```{r disp-abs-diff, echo=TRUE, eval=TRUE, fig.width=5.2, fig.height=3.2, fig.cap="Absolute difference in acceptance rate (percentage points) versus Private insurance. Negative values indicate lower appointment-offered rates.", purl=FALSE}
mysterycall_plot_disparities(disp_tbl, metric = "abs_diff",
                              title = "Absolute disparity (pp vs. Private)")
```

```{r disp-rr, echo=TRUE, eval=TRUE, fig.width=5.2, fig.height=3.2, fig.cap="Relative risk of being offered an appointment versus Private insurance. Values below 1 indicate lower rates for that group.", purl=FALSE}
...
```{r disparities-basic, eval=FALSE, purl=FALSE}
...
```{r disparities-ref, eval=FALSE, purl=FALSE}
...
Private-reference table would not emphasize.
...
```{r disparities-error, eval=FALSE, purl=FALSE}
...
```{r multiple-comparison, eval=FALSE, purl=FALSE}
...
```{r multiple-comparison-full, eval=FALSE, purl=FALSE}
...
```{r bootstrap-ci, eval=FALSE, purl=FALSE}
...
```{r bootstrap-custom, eval=FALSE, purl=FALSE}
...
```{r write-results, eval=FALSE, purl=FALSE}
...
```{r build-data, eval=FALSE, purl=FALSE}
set.seed(42)
...
```{r full-preflight, eval=FALSE, purl=FALSE}
...
```{r full-disparities, eval=FALSE, purl=FALSE}
...
```{r full-poisson, eval=FALSE, purl=FALSE}
...
```{r full-wait-summary, eval=FALSE, purl=FALSE}
...
```{r full-bootstrap, eval=FALSE, purl=FALSE}
set.seed(20240101)
...
```{r pub-table, eval=FALSE, purl=FALSE}
...
```{r full-write-results, eval=FALSE, purl=FALSE}
...
```{r cochran-n, echo=TRUE, eval=TRUE, purl=FALSE}
...
For the **count outcome** (wait days) analyzed with a Poisson GLMM, power
...
```{r poisson-power, echo=TRUE, eval=TRUE, purl=FALSE}
...
```{r power-curve, echo=TRUE, eval=TRUE, fig.width=5.5, fig.height=3.5, fig.cap="Required providers per arm (paired design, 80 % power) as a function of the minimum detectable IRR. Detecting small effects (IRR close to 1) requires substantially more providers.", purl=FALSE}
