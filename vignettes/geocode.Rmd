...
> **Moved to `mysterymaps`.** Geocoding has moved to the companion
> [`mysterymaps`](https://github.com/mufflyt/mysterymaps) package
> (`mysterymaps::mysterymaps_geocode()`). It is no longer part of
> `mysterycall`; this article is kept for reference only and the code below
> will not run against `mysterycall`.
...
Combine it with `mysterycall_search_taxonomy()` to build a fully enriched
