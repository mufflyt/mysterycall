...
is to harmonize the heterogeneous column names that inevitably accumulate
across five or ten individual Excel files and to normalize the free-text
...
```{r one-call, eval=FALSE, purl=FALSE}
...
```{r phase1-params, eval=FALSE, purl=FALSE}
...
```{r phase1-flag-example, eval=FALSE, purl=FALSE}
...
```{r phase1-attrs, eval=FALSE, purl=FALSE}
...
```{r phase1-error, eval=FALSE, purl=FALSE}
...
```{r npi-validate, eval=FALSE, purl=FALSE}
...
```{r npi-dups, eval=FALSE, purl=FALSE}
...
```{r split-params, eval=FALSE, purl=FALSE}
...
their workbook. This is useful for standardizing the call script order.
...
```{r workload-bar, echo=TRUE, eval=TRUE, fig.width=5, fig.height=2.8, fig.cap="Example round-robin workload distribution across five callers for a 198-provider roster. Differences of at most 1 provider guarantee balanced calling burden.", purl=FALSE}
...
```{r split-verify, eval=FALSE, purl=FALSE}
...
must be merged and standardized. This is Phase 2. The core challenge is that
...
You provide a list of target substrings to look for and the standardized
names to replace them with.

## 5.2 `mysterycall_rename_columns()`: fuzzy column name matching

```{r rename-cols, eval=FALSE, purl=FALSE}
...
Each element is the standardized name to assign when the corresponding
substring is found.
...
```{r rename-warning, eval=FALSE, purl=FALSE}
...
```{r phase2-clean, eval=FALSE, purl=FALSE}
...
```{r phase2-full, eval=FALSE, purl=FALSE}
...
```{r quality-params, eval=FALSE, purl=FALSE}
...
```{r quality-flag, eval=FALSE, purl=FALSE}
...
```{r coverage-params, eval=FALSE, purl=FALSE}
...
```{r coverage-table, eval=FALSE, purl=FALSE}
...
```{r coverage-filter, eval=FALSE, purl=FALSE}
...
```{r workflow-list, eval=FALSE, purl=FALSE}
names(result)
...
```{r workflow-full, eval=FALSE, purl=FALSE}
result <- mysterycall_run_workflow(
  taxonomy_terms      = c(
...
```{r workflow-logged, eval=FALSE, purl=FALSE}
...
```{r workflow-summary, eval=FALSE, purl=FALSE}
knitr::kable(
...
because they realized a phone number was a fax line or the provider had
...
```{r workflow-check, eval=FALSE, purl=FALSE}
...
```{r preflight, eval=FALSE, purl=FALSE}
...
```{r large-study, eval=FALSE, purl=FALSE}
...
```{r business-days-scalar, echo=TRUE, eval=requireNamespace("bizdays", quietly = TRUE), purl=FALSE}
...
```{r business-days-df, echo=TRUE, eval=requireNamespace("bizdays", quietly = TRUE), purl=FALSE}
...
```{r caller-reliability, echo=TRUE, eval=TRUE, purl=FALSE}
set.seed(7)
...
```{r call-productivity, echo=TRUE, eval=TRUE, purl=FALSE}
set.seed(8)
n_rows <- 80L
prod_df <- data.frame(
