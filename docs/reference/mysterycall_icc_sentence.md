...
Formats the ICC result from
[`mysterycall_icc()`](https://mufflyt.github.io/mysterycall/reference/mysterycall_icc.md)
into a ready-to-paste methods sentence.

## Usage

``` r
mysterycall_icc_sentence(icc_result)
```
...
- icc_result:
  ...
  A `mysterycall_icc` object.

## Value

A single character string suitable for the methods section.
...
#> The intraclass correlation coefficient was 0.180 (18.0% of variance attributable to between-physician clustering), justifying the use of a multilevel model.
```
