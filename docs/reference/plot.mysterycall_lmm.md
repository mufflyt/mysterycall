...
model residuals and a residuals-vs-fitted plot. Use these **before**
relying on the LMM - if the Q-Q plot shows a systematic S-shape or heavy
tails, switch to a Poisson or negative-binomial GLMM.

## Usage

``` r
# S3 method for class 'mysterycall_lmm'
plot(x, ...)
```
...
A `ggplot` object (invisibly). The plot is also printed to the active
graphics device.
